...
-- lua/util/capabilities.lua is hand-kept; this checks the half a machine
-- can: the first key of each entry is bound, or the entry describes
-- something that no longer exists.

local capabilities = require("util.capabilities")

-- In a buffer with a language server attached, since the list includes
-- per-buffer LspAttach keys.
