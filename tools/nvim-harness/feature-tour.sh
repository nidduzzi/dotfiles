...
SCENARIOS=(
  # -- discovery -------------------------------------------------------------
  "startup|Dashboard on an empty start|8|Neovim loaded|"
  "whichkey-leader|Key hints: leader menu|12|f ➜|Space|ff|lib.lua|Enter|Escape|Space"
  "whichkey-search|Key hints: the search group|12|w ➜|Space|ff|lib.lua|Enter|Escape|Space|s"
  "whichkey-goto|Key hints: the goto prefix|12|Move to|Space|ff|lib.lua|Enter|Escape|g"
  # -- finding files and text ------------------------------------------------
  "find-files|Search files|8|Files|Space|ff"
  "grep-code|Search file contents, documentation excluded|8|Grep \(code\)|Space|sg|validateToken"
  "grep-all|Search file contents, everything|12|Grep \(all\)|Space|sg|validateToken|M-S|all|Enter"
  "grep-docs|Search documentation only|12|Grep \(docs\)|Space|sg|validateToken|M-S|docs|Enter"
  "grep-word|Search the word under the cursor|12|Grep \(code\)|Space|ff|auth.js|Enter|:3|Enter|w|Space|sw"
  "grep-buffer|Search lines in the current buffer|12|Lines|Space|ff|auth.js|Enter|Space|sb"
  "grep-open|Search across open buffers|14|Grep Buffers|Space|ff|auth.js|Enter|Space|ff|lib.lua|Enter|Space|sB"
  "grep-tree|Results grouped by file in Trouble|8|Snacks|Space|sg|validateToken|wait:3:C-t"
  "fuzzy-files|Fuzzy matching in the file picker|10|auth.js|Space|ff|athjs"
  "fuzzy-toggle|Grep, then C-g to fuzzy filter the results|12|README|Space|sg|validateToken|C-g|README"
  "fuzzy-path|Fuzzy filtering the results by path|12|src/|Space|sg|validateToken|C-g|src/"
  "regex-default|Regex is the default: token.*expiry matches, as ripgrep would|12|expiresAt|Space|sg|token.*expiry"
  "regex-toggle|a-r switches to fixed-string matching, shown by R in the title|14|No results|Space|sg|M-r|token.*expiry"
  ...
  "filter-choose|Choosing a filter preset from a list with a-S|12|Search filter|Space|sg|validateToken|M-S"
  "resume|Resume the last search|14|Grep \(code\)|Space|sg|validateToken|wait:2:Escape|Space|sR"
  "buffers|Buffer list|12|Buffers|Space|ff|lib.lua|Enter|Space|ff|app.py|Enter|Space|,"
  "recent|Recent files|8|Recent|Space|fr"
  "help-tags|Help tags|8|Help|Space|sh"
  "keymaps|Keymaps|8|Keymaps|Space|sk"
  "marks|Marks|8|Marks|Space|sm"
  "explorer|File tree explorer|8|Explorer|Space|e"
  "explorer-search|Searching inside the tree: i focuses the filter, the tree stays a tree|12|lib.lua|Space|e|i|lua"
  "explorer-search-deep|A filter matching inside nested directories, shown in place|12|openspec|Space|e|i|spec"
  ...
  "lsp-diagnostics|Diagnostics for the buffer|16|Diagnostics|Space|ff|broken.py|Enter|Space|xx"
  ...
  "git-hunk|Git hunk preview|14|validateToken\(user\.token\);|Space|ff|login.js|Enter|G|Space|ghp"
  "git-blame|Git blame for the line|14|Git Log Line|Space|ff|login.js|Enter|:1|Enter|Space|gb"
  "git-status|Changed files|10|Git Status|Space|gs"
  "git-log|Commit log|10|Git Log|Space|gl"
  # -- editing ---------------------------------------------------------------
  "todo|TODO, FIXME and HACK comments|8|Todo Comments|Space|st"
  "yank-history|Yank ring history|10|Yank History|Space|ff|lib.lua|Enter|yy|Space|sy"
  "treesitter|Syntax highlighting|12|function validateToken|Space|ff|auth.js|Enter"
  "folds|Folded code|12|lines|Space|ff|auth.js|Enter|zM"
  "terminal|Terminal|10|terminal works|Space|ft|echo terminal works|Enter"
  "debug-ui|Debugger breakpoint and menu|14|➜|Space|ff|app.py|Enter|:5|Enter|Space|db|Space|d"
  # -- the editor itself -----------------------------------------------------
  "lazy|Plugin manager|10|lazy.nvim|Space|l"
  "mason|Tool installer|10|Language Filter|Space|cm"
  "colorscheme|Colourscheme picker|8|Colorschemes|Space|uC"
  "health|Which language servers this project provides|14|language servers|:checkhealth dotfiles|Enter"
  "notifications|Notification history|12|Notifications|Space|sg|validateToken|M-c|Escape|wait:2:Space|n"
)
